...
struct PQUF {
    ...
    ii pop() {
        int l = ps.rbegin()->second;
        
        int ci = stk.size(); 
        
        if(ci-l > 600) {
            // Rebuild UF
            vector<tuple<int,int,int>> cstk = stk;
            stk.clear();
            sort(ALL(cstk));
            U = UF(n);
            FORE(x,cstk) {
            ...
            int hhp = ps.rbegin()->first;
            vector<tuple<int,int,int>> tstk;
            while(get<0>(stk.back()) != hhp) {
                tstk.push_back(stk.back());
                stk.pop_back();
                U.undo();
            }
            int hp, hu, hv; 
            tie(hp,hu,hv) = stk.back();
            stk.pop_back();
            U.undo();
            sort(ALL(tstk));
            FORE(x,tstk) {
